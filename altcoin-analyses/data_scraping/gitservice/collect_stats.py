...
def get_repo_lists(single_only=False, multiple_only=False):
    ...
    logging.debug("Getting Repo Lists, single_only = %s , multiple_only= %s", single_only, multiple_only)
...
def paginate_repos(input_data, org_list, depth=0):
    query = ""
    cursors = {}
    orgs_to_paginate = [(name, data) for name, data in input_data['data'].iteritems()
                        if len(data['repositories']['edges']) == 100]
    orgs_to_paginate_names = [name for name, data in input_data['data'].iteritems()
                        if len(data['repositories']['edges']) == 100]
    if not orgs_to_paginate:
        return input_data
    logging.debug("Github orgs/users being paginated are %s", orgs_to_paginate_names)

    #Create graphql query to paginate orgs
